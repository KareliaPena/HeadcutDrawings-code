...
def JFA(img2,nsamples):
    ...
    for i in range(sqrtNSamples):
        for j in range(sqrtNSamples):
            offsets[i*sqrtNSamples+j, :] = [(i+np.random.rand())*jstep, (j+np.random.rand())*jstep];
...
def FVI(img):
    ...
    lambda1_filled,lambda2_filled = structure_tensor_eigvals(Axx_filled, Axy_filled, Ayy_filled)
    ...
    Ang_v2[v2_x_filled_N!=0]=np.arctan(np.divide(v2_y_filled_N[v2_x_filled_N!=0],v2_x_filled_N[v2_x_filled_N!=0]))
    ...
    Ang_v1[v1_x_filled_N!=0]=np.arctan(np.divide(v1_y_filled_N[v1_x_filled_N!=0],v1_x_filled_N[v1_x_filled_N!=0]))
        ...
        while ite < maxite:
            for y in range(In_N.shape[0]):
                for x in range(In_N.shape[1]):
                    rotated, pxN  = rotate_bound(In_T_F, -Ang_v2[y,x]*360/(2*np.pi), np.array([x,y,1]))
                    ...
                    for m in range(2*d+1):
                        x_m=m/2
                        ip=i+x_m
                        S_x = S_Reflection(np.reshape(ip,(i.size,1)),d*np.ones((i.size,1)))
                    ...
                    x_mf=minInd/2
                    ipf=i+x_mf
                    S_x = S_Reflection(np.reshape(ipf,(ipf.size,1)),d*np.ones((ipf.size,1)))
            ...
            for y in range(In_N.shape[0]):
                for x in range(In_N.shape[1]):
                    rotated, pxN  = rotate_bound(In_N_F, -Ang_v1[y,x]*360/(2*np.pi), np.array([x,y,1]))
                    infX_L = np.maximum(pxN[0] - r-1,0)
                    infX_H = np.minimum(pxN[0] + r,rotated.shape[1])
                    infY_L = np.maximum(pxN[1] - r-1,0)
                    infY_H = np.minimum(pxN[1] + r,In_N.shape[0])
