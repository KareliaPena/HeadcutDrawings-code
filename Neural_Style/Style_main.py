...
def load_seg(content_seg_path, style_seg_path, content_shape, style_shape):
    ...
    def _extract_mask(seg, color_str):
        ...
        if color_str == "BLUE":
            ...
            mask_r = np.multiply(
                (seg[:, :, 0] > 0.4).astype(np.uint8),
                (seg[:, :, 0] < 0.6).astype(np.uint8),
            )
            mask_g = np.multiply(
                (seg[:, :, 1] > 0.4).astype(np.uint8),
                (seg[:, :, 1] < 0.6).astype(np.uint8),
            )
            mask_b = np.multiply(
                (seg[:, :, 2] > 0.4).astype(np.uint8),
        ...
        return np.multiply(np.multiply(mask_r, mask_g), mask_b).astype(
            np.float32
        )  # this is an AND among the three channels

    # PIL resize has different order of np.shape
    content_seg = (
        np.array(
            Image.open(content_seg_path)
            .convert("RGB")
            .resize(content_shape, resample=Image.BILINEAR),
            dtype=np.float32,
        )
        / 255.0
    )
    style_seg = (
        np.array(
            Image.open(style_seg_path)
            .convert("RGB")
            .resize(style_shape, resample=Image.BILINEAR),
            dtype=np.float32,
        )
        / 255.0
    )
    ...
    for i in range(len(color_codes)):
        color_content_masks.append(
            tf.expand_dims(
                tf.expand_dims(
                    tf.constant(_extract_mask(content_seg, color_codes[i])), 0
                ),
                -1,
            )
        )
        color_style_masks.append(
            tf.expand_dims(
                tf.expand_dims(
                    tf.constant(_extract_mask(style_seg, color_codes[i])), 0
                ),
                -1,
            )
        )
    ...
    for i in range(len(layers_names)):
        ...
        if "pool" in layer_name:
            content_seg_width, content_seg_height = int(
                math.floor(content_seg_width / 2)
            ), int(math.floor(content_seg_height / 2))
            style_seg_width, style_seg_height = int(
                math.floor(style_seg_width / 2)
            ), int(math.floor(style_seg_height / 2))
            ...
            for i in range(len(content_segs)):
                ...
                style_segs[i] = tf.image.resize_bilinear(
                    style_segs[i], tf.constant((style_seg_height, style_seg_width))
                )
            ...
            for i in range(len(content_segs)):
                ...
                content_segs[i] = tf.nn.avg_pool(
                    tf.pad(
                        content_segs[i], [[0, 0], [1, 1], [1, 1], [0, 0]], "CONSTANT"
                    ),
                    ksize=[1, 3, 3, 1],
                    strides=[1, 1, 1, 1],
                    padding="VALID",
        ...
        if layer_name in style_layers:
            ...
            for content_seg, style_seg in zip(content_segs[1:], style_segs[1:]):
                ...
                gram_matrix_const = tf.cond(
                    tf.greater(style_mask_mean, 0.0),
                ...
                )  # Multiplication of each feature map by the segmentation mask
                Var_FeatureMaps = tf.multiply(var_layer, content_seg)
                content_mask_mean = tf.reduce_mean(
                    content_seg
                )  # The mean indicates the amount of values different from zero in the segmented mask
                gram_matrix_var = tf.cond(
                    tf.greater(content_mask_mean, 0.0),
                    lambda: gram_matrix_var
                    / (
                        tf.to_float(tf.size(var_layer)) * content_mask_mean
                    ),  # it is a ponderated sum so it is divided by the number of elements different from zero in the segmented mask
                    lambda: gram_matrix_var,  # in this case should be zero
                )

                diff_style_sum = (
                    tf.reduce_mean(
                        tf.squared_difference(gram_matrix_const, gram_matrix_var)
                    )
                    * content_mask_mean
                )  # It should be multiplied by the nonzero positions on
...
def load_mask(mask_path, th):
    ...
    thresh1 = np.expand_dims(np.expand_dims(thresh1, axis=0), axis=3)
    thresh1 = tf.image.resize_bilinear(
        thresh1, tf.constant((content_height, content_width))
    )
...
def load_mask2(mask_path, th):
    ...
    final = tf.image.resize_bilinear(
        final, tf.constant((content_height, content_width))
    )
...
def deprocess_img2(processed_img):
    ...
    x = np.append(
        x, np.expand_dims(np.squeeze(processed_img[:, :, :, 1] + 116.779, 0), 2), axis=2
    )
    x = np.append(
        x, np.expand_dims(np.squeeze(processed_img[:, :, :, 2] + 123.68, 0), 2), axis=2
    )
...
def get_content_loss(base_content, target, edges):
    edges2 = tf.image.resize_bilinear(
        edges, tf.constant((target.shape[1], target.shape[2]))
    )
    return tf.reduce_mean(
        tf.square(tf.multiply(tf.expand_dims(base_content, 0) - target, edges2))
    )
...
def get_Binary_Mask_loss(base_content, Grid_image, Black_Mask):
    return tf.reduce_mean(
        tf.square(
            tf.multiply(tf.multiply(Grid_image, 1 - Black_Mask), 255 - base_content)
        )
    )
...
def get_feature_representations(
    ...
    content_masks, style_masks = load_seg(
        content_seg_path,
        style_seg_path,
    ...
    style_features = [
        style_outputs[style_layers_idx[i]] for i in range(len(style_layers_idx))
    ]
    content_features = [
        content_outputs[content_layers_idx[i]] for i in range(len(content_layers_idx))
    ]
    mask_features = [
        mask_outputs[mask_outputs_idx[i]] for i in range(len(mask_outputs_idx))
    ]
...
def compute_loss(
    ...
    style_output_features_var = [
        model_outputs[style_layers_idx[i]] for i in range(len(style_layers_idx))
    ]
    content_output_features_var = [
        model_outputs[content_layers_idx[i]] for i in range(len(content_layers_idx))
    ]
    mask_output_features_var = [
        model_outputs[mask_idx[i]] for i in range(len(mask_idx))
    ]
    ...
    style_score_list, style_mask_score_list = segment_style_loss(
        model,
        layers_names,
        style_layers,
        mask_layers,
    ...
    for target_content, comb_content in zip(
        content_features, content_output_features_var
    ):
        content_score += weight_per_content_layer * get_content_loss(
            comb_content[0], target_content, mask_edges
        )
    ...
    for target_content, comb_content in zip(mask_features, mask_output_features_var):
        content_mask_score += weight_per_content_mask_layer * get_content_loss(
            comb_content[0], target_content, 1 - mask_edges
        )
    ...
    for jj in range(len(image_filesContent)):
        for ll in range(len(image_filesStyles)):
            ...
            for ii in range(len(Ratio_List)):
                ...
                _, edgesST, InitialImage = init_mix_image(
                    content_path, mask_path, alpha
                )

                ite = 0
                best, best_loss, imgs, losses = run_style_transfer(
                    content_path,
                    style_path,
                    mask_path,
                    num_iterations,
                ...
                out = np.multiply(
                    inner2.astype("float64"), content_outline.astype("float64")
                )
                ...
                sio.savemat(
                    dirName + "C_%04d_S_%04d_R_%02d.png" % (jj, ll, ii) + ".mat",
                    {
                        ...
                        "alpha": alpha,
                        "content_mask_weight": content_mask_weight,
                        "style_mask_weight": style_mask_weight,
